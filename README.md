...
| `mypage.html` | My / 설정 | 프로필, 포트폴리오 현황, 알림 토글, **이메일/비밀번호 변경**, 로그아웃 |
| `admin.html` | 🆕 관리자 대시보드 | 슈퍼관리자 전용 — 회원 명단, 자금 내역, 투자 현황, 정산 이력, 계정 설정 |
| `benchmark.html` | 벤치마크 비교 | 알고리즘 성과 비교, 시나리오 분석 |
| `login.html` | 로그인 | 이메일+비밀번호 로그인, 관리자 로그인 시 admin.html 자동 이동 |

| `login.html` | 로그인 | 저장 계정 카드, 비밀번호 토글, 비밀번호 재설정 모달, **관리자 로그인 → admin.html 이동** |
...
- `js/vie-app.js` — 핵심 앱 로직 (API, Auth, Simulator, Counter, Toast, Chart 헬퍼, **VieAdmin 관리자 로직**, **updateCredentials**)
...
### 접근 방법
1. `login.html`에서 `valuencores@gmail.com` / `@vnc1201` 로그인
2. 로그인 성공 시 `admin.html`로 **자동 이동**
3. 또는 모든 페이지 **우측 하단의 ADMIN 플로팅 버튼** 클릭 (관리자 로그인 상태일 때만 표시)

### 패널 구성
| 패널 | 기능 |
|------|------|
| 전체 현황 | KPI 카드 (회원수·원금·기지급배당·총자산), 회원 요약 테이블, 자산 도넛 차트, 다음 정산 예정 |
| 회원 명단 | 이름·이메일·휴대폰·파트너·포트폴리오·이메일알림·카카오알림·약관동의·상태 + 상세 모달 |
| 자금 내역 | 회원별 투자원금·월수익률·월수익금·기지급·총자산 |
| 투자 현황 | 운용기간·원금·누적수익금·누적수익률·알고리즘 |
| 정산 이력 | 전체 정산 이력 (지급완료/예정 상태 구분) |
| 계정 설정 | 이메일/비밀번호 변경, 이메일·카카오 알림 토글, 이용약관·개인정보 처리방침 동의 현황 |
...
| `/mypage.html` | 로그인 필요 | My/설정 (이메일·비밀번호 변경 포함) |
| `/login.html` | 누구나 | 로그인 |
| `/admin.html` | **superadmin 전용** | 관리자 대시보드 (미인증 시 login.html 리다이렉트) |
